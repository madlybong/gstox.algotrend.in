...
async function validate(values: any, { setErrors }: any) {
    ...
    return authStore.login(username.value, password.value).catch((error) => { setErrors({ apiError: error }); console.log("apiError: ", error); });
}
</script>
...
<template>
    <Form @submit="validate" v-slot="{ errors, isSubmitting }" class="mt-5">
        ...
        <div class="d-flex gc-3 flex-wrap align-center my-3 ml-n2">
